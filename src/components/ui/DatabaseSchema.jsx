    ...
    <div
        ...
        {columns.map((col, idx) => (
          <div key={idx} className="flex items-center justify-between gap-4 text-sm font-mono">
            <div className="flex items-center gap-2">
              {col.isPK && (
                <Key size={14} className="text-amber-500 dark:text-dracula-yellow flex-shrink-0" />
              )}
              {col.isFK && (
                <Link2 size={14} className="text-cyan-600 dark:text-dracula-cyan flex-shrink-0" />
              )}
              <span
                className={`
            ...
            <span className="text-gray-400 dark:text-dracula-comment text-xs">{col.type}</span>
          </div>
        ))}
      </div>
      ...
      <div className="absolute -top-2.5 -right-2.5 flex gap-1">
        {columns.some((c) => c.isPK) && (
          <span className="px-2 py-0.5 text-[10px] font-bold bg-amber-500 dark:bg-dracula-yellow text-white dark:text-dracula-background rounded-full">
            PK
          </span>
        )}
        {columns.some((c) => c.isFK) && (
          <span className="px-2 py-0.5 text-[10px] font-bold bg-cyan-500 dark:bg-dracula-cyan text-white dark:text-dracula-background rounded-full">
            FK
          </span>
        )}
      </div>
    ...
    <div
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-gray-100 dark:bg-dracula-current border-b border-gray-300 dark:border-dracula-comment">
        <div className="flex items-center gap-2">
          ...
          <span className="text-gray-900 dark:text-dracula-foreground text-xs font-mono font-semibold">
            Database Schema
          </span>
          <span className="text-gray-500 dark:text-dracula-comment text-xs">
            — FoodieExpress System (Project 1)
          </span>
        </div>
        <div className="flex items-center gap-3 text-xs font-mono text-gray-500 dark:text-dracula-comment">
